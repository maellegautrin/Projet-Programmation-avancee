...
impl<'ast> LEnv<'ast> {
    ...
    fn lookup(&self, name: &Name) -> Option<&RefCell<Value<'ast>>> {
        match self {
            LEnv::Nil => None,
            LEnv::Cons(scope, rest) => match scope.get(name) {
    ...
    // par défaut fait par le compilateur Rust ne permet pas d'implémenter
    // correctement l'interpréteur.
    pub fn extend<V>(self: &Rc<Self>, names: &'ast [Name], mut values: V) -> Rc<Self>
    where V: Iterator<Item = Value<'ast>>,{
        let mut new_scope = Scope::new();
        for name in names {
            new_scope.insert(name, RefCell::new(values.next().unwrap_or(Value::Nil)));
        }
        Rc::new(LEnv::Cons(new_scope, Rc::clone(self)))
    }
}
...
impl<'ast, 'genv> Env<'ast, 'genv> {
    ...
    pub fn lookup(&self, name: &Name) -> Value<'ast> {
        if let Some(value_ref) = self.locals.lookup(name) {
            return value_ref.borrow().clone();
        }
        if let Some(value) = self.globals.0.get(name) {
            return value.clone();
        }
        Value::Nil
    }
